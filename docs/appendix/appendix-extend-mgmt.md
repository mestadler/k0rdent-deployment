...
You can override the default registry settings in k0rdent by specifying the `templatesRepoURL`, `insecureRegistry`,
...
You can override the default URL from which to download the k0s binary in k0rdent by
...
> WARNING: Prior to `v1.2.0` k0rdent, this is not supported for the `k0sproject-k0smotron` provider due to a bug in the CAPI Operator:
...
>
> Starting `v1.2.0` k0rdent shipped with a CAPI Operator version with [the issue addressed](https://github.com/kubernetes-sigs/cluster-api-operator/pull/796).
...
k0rdent starting `v1.1.0` and is *disabled* by default.
