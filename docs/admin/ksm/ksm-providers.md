...
k0rdent provides API to make underlying CD software pluggable and leverage it for KSM purposes.
There are two crucial API types: `StateManagementProvider` and `ServiceSet`.

## StateManagementProvider

The `StateManagementProvider` ensures that `ServiceSet` objects produced by k0rdent controllers
...
Default k0rdent provider is ProjectSveltos. k0rdent embeds adapter
