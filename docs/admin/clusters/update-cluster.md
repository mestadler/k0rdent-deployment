...
As you can see from the `.spec`, the `aws-standalone-cp-1-8-0` template can be applied to a cluster that also uses
the `aws-standalone-cp-1-8-0` template, or it can be used as an upgrade from a cluster that uses `aws-standalone-cp-0.0.2`.
